...
void Player_Defense(Player* player)
{
	if (player->Energy >= player->DefenseEnergyNeeded)
	{
		...
		sprintf(Statement, "이번 턴에 방어도 %d 획득", tempArmour);
	}
}
...
void DrawPlayerDebuff(Player* player)
{
	if (player->IsWeakened)
	{
		...
		sprintf(PlayerDebuffStr, "약화 : %d", player->RemainedWeakness);
...
void DrawPlayerStatus(Player* player)
{
	...
	sprintf(PlayerAttackStr, "공격력 : %d", player->Attack);
	sprintf(PlayerDefenseStr, "수비력 : %d", player->Defense);
	sprintf(PlayerEnergyStr, "에너지 : %d / %d", player->Energy, player->MaxEnergy);
	...
	{
		char PlayerHasUniqueRewardStr[24] = "유니크 보상 보유";
